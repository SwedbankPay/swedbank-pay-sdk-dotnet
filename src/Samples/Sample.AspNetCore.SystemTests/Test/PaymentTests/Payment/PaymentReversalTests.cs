    ...
    public class PaymentReversalTests : Base.PaymentTests
    {
        ...
        public async Task Payment_Card_Reversal(Product[] products, PayexInfo payexInfo)
        {
            ...
            Assert.That(order.PaymentResponse.Transactions.TransactionList.First(x => x.Type == "Capture").State.Value,
                        Is.EqualTo(State.Completed));
            Assert.That(order.PaymentResponse.Transactions.TransactionList.First(x => x.Type == "Reversal").State.Value,
                        Is.EqualTo(State.Completed));
        }
        ...
        public async Task Payment_Swish_Reversal(Product[] products, PayexInfo payexInfo)
        {
            ...
            var order = await SwedbankPayClient.Payment.GetSwishPayment(paymentLink, SwedbankPay.Sdk.Payments.PaymentExpand.All);
            ...
            Assert.That(order.PaymentResponse.Transactions.TransactionList.First(x => x.Type == "Sale").State.Value,
                        Is.EqualTo(State.Completed));
            Assert.That(order.PaymentResponse.Transactions.TransactionList.First(x => x.Type == "Reversal").State.Value,
                        Is.EqualTo(State.Completed));
        }
